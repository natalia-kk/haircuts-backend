...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('favouriteHaircuts') // this talks to the User model:
                                                            // instead of the favouriteHaircuts just being ids - it'll now populate them with the haircut object
    .then(user => {
      res.json(user)
    })
    .catch(err => {
...
router.put('/:id', Utils.authenticateToken, (req, res) => {
  ...
  if(req.files && req.files.avatar){
    ...
    updateUser(req.body) // only update fields passed in by user
  }
